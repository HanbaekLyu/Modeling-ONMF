...
from utils.time_series_ONMF import ONMF_timeseries_reconstructor
import numpy as np
...
def main_train_joint():
    ...
    full_state_list_train = ['Wisconsin']  ### List of states for learning dictionary from
    full_state_list_test = ['Wisconsin']  ### List of states for transfer-prediction
    ...
    title = full_state_list_test[0] + " (" + "transfer prediction using dictionary learned from " + str(list_train) + ") \n" + "moving window size= " + str(moving_window_size) + ", 1-step prediction= " + str(onestep_prediction_length) + " days ahead" + ", " + "total prediction: " + str(future_extrapolation_length*onestep_prediction_length) +" days ahead"
